...
string Room::Update(Uint32 milliSec, IGameState *lastState)
{
	bool playerWasKilled = false;

	size_t count = _updatables.size();
    size_t ii;
	...
	{
		IUpdatable *current = _updatables[ii];
...
void Room::pickCoins(int tileX1, int tileX2, int tileY1, int tileY2)
{
	...
	if(this->_monedas.size() == 0 && !(this->_player->getEstado() & (TodasMonedasCogidas | Muriendo)))
	{
